...
| **标签打印+扫码出库** | ✅ 已完成 | Code128条码生成 + 标签打印页 + 手机扫码出库页 + SKU查询API |
| **利润测算定价生成器** | ✅ 已完成 | 历史数据统计 + 定价推荐引擎 + 风险评估 + 器型对比 |
| **后端代码质量优化** | ✅ 已完成 | 销售编号生成重构(提取公共函数) + 压货预警DB层过滤 + 缩进bug修复 |
| **前端功能优化** | ✅ 已完成 | 库存列表批量打印标签入口 + 新窗口打印 |
| **Docker 部署配置** | ✅ 已完成 | Dockerfile(多阶段构建) + docker-compose.yml + .dockerignore + .env.example |
...
**P2 扩展功能（完成度 ~80%）**
- [x] 标签打印 — Code128 条码生成 + 打印标签页 + 入库详情页打印按钮 + 库存列表批量打印
- [x] 扫码出库 — 手机摄像头扫描 + SKU 查询 + 快速出库表单
- [x] 利润测算定价生成器 — 历史数据统计 + 定价推荐 + 风险评估 + 器型对比
- [x] Docker 部署 — Dockerfile(多阶段构建) + docker-compose.yml + .env.example
- [ ] 镶嵌定制业务（独立模块，未开始）
- [x] 借出管理功能 — PRD 标注"暂不纳入核心开发"

### 后端路由模块（11个文件，12个router对象，53个端点）
...
routers/label.py        — 标签打印 + Code128条码生成 + SKU扫码查询（4端点）
routers/pricing.py      — 利润测算定价生成器 + 历史数据统计 + 定价推荐（5端点）
...
views/InventoryDetail.vue    — 库存详情（全部字段+规格+图片上传/删除/设封面+出库+打印标签按钮）
...
views/LabelPrint.vue         — 标签打印（批量标签数据+Code128条码+CSS @media print打印）
views/ScanSell.vue           — 扫码出库（手机摄像头扫码+SKU查询+快速出库表单）
views/PricingCalculator.vue  — 利润测算定价生成器（成本输入+历史统计+定价推荐+风险评估）
```

**组件（9个）**：
```
components/AppLayout.vue       — 应用布局（桌面下拉导航+移动端底栏含扫码突出按钮，工具菜单含扫码/标签/字典/贵金属/供应商）
...
| **P3** | 标签打印优化 | 功能优化 | 自定义标签模板 + 热敏打印机支持（ESC/POS指令） |
| **P3** | 增加测试覆盖 | 质量保障 | 后端 pytest + 前端 Vitest |
| **P3** | 极空间 NAS 部署 | 运维 | docker-compose up + 数据卷持久化 + 反向代理配置 |
...
| 条码生成 | python-barcode（Code128 条码 PNG 生成） |
| Excel 导出 | openpyxl |
